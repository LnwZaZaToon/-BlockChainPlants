...
const UserPage = () => {
  ...
  const [profileType, setProfileType] = useState('admin'); // Default to 'user', but can be fetched from API or context

  useEffect(() => {
    // Fetching the data when the component is mounted
    const fetchData = async () => {
      try {
        const response = await fetch('/api/user');  // Replace with the actual API endpoint
        const result = await response.json();
        const filteredResult = result.filter(item => item.granted === "admin");
